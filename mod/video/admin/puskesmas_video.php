...
js;
	...
	if(!isset($_GET['action'])){
		...
		<div class="breadcrumb"><a href="puskesmas.php?mod=video" id="home">Home</a>   &nbsp;&raquo;&nbsp;   Video Manager</div>
		</div>		
		<div class="sorts">
			<div id="tabs">
				<ul>
				...
				<div id="tabs-1">';
				$a 		= $db->sql_query("SELECT * FROM `mod_video` WHERE  `username`='$username' ORDER BY `date` DESC");
				...
				$b 		= $db->sql_query("SELECT * FROM `mod_video` WHERE  `username`='$username' ORDER BY `date` DESC LIMIT $offset,$limit");
				...
				while($data = $db->sql_fetchrow($b)){
					...
					$published 	= ($data['published'] == 1) ? '<a class="enable" href="#" title="Sudah di Verifikasi Admin">Sudah di Verifikasi Admin</a>' : '<a class="disable" href="#" title="Belum di Verifikasi Admin">Belum di Verifikasi Admin</a>';
					...
					<tr'.$warna.'>
			            ...
			            <td style="text-align: center;"><a class="edit" href="puskesmas.php?mod=video&amp;action=edit&amp;id='.$data['id'].'" title="Edit">Edit</a> <a class="delete" href="puskesmas.php?mod=video&amp;action=delete&amp;id='.$data['id'].'" onclick="return confirm(\'Are You Sure deleted this ?\')" title="Delete">Delete</a></td>
					</tr>';
					$no++;					
				}
				$tengah .= '
				...
				<div id="tabs-2">';
					...
					if(isset($_POST['submit'])){
						...
						if($error){
							...
							$success = $db->sql_query("INSERT INTO `mod_video` (`title`,`description`,`code`,`date`,`published`,`seftitle`) VALUES ('$title','$content','$code','$date','0','$seftitle')");
							if($success){
								$tengah .= '<div class="success">Video Has Been Added in Database</div>';
								unset($title);
								unset($content);
								unset($code);
								$style_include[] = '<meta http-equiv="refresh" content="1; url=puskesmas.php?mod=video" />';
							}else{
	...
	if($_GET['action'] =='edit'){
		...
		<div class="breadcrumb"><a href="puskesmas.php?mod=video" id="home">Home</a>   &nbsp;&raquo;&nbsp;   <a>Video Manager</a></div>
		</div>';
			...
			if(isset($_POST['submit'])){
				...
				if($error){
					...
					if($success){
						...
						$style_include[] = '<meta http-equiv="refresh" content="0; url=puskesmas.php?mod=video" />';						
					}else{
						$tengah .= '<div class="error">'.mysql_error().'</div>';
					}					
				}						
			}
			$data    	= $db->sql_fetchrow($db->sql_query("SELECT * FROM `mod_video`  WHERE  `username`='$username' AND `id`='$id'"));
			$title 	 	= $data['title'];
			$code 	 	= $data['code'];
			$content 	= $data['description'];
			
			if($data['published'] == 1){
				$tengah .= '<div class="error">Maaf data tidak bisa di Edit Karena sudah di Verifikasi oleh Administrator</div>';
			}else{
	...
	if($_GET['action'] == 'delete'){
		$id 		= int_filter($_GET['id']);
		$delete = mysql_query("DELETE FROM `mod_video`  WHERE  `username`='$username' AND `id` = '$id'");
		if ($delete) {
			header("location: puskesmas.php?mod=video");
			exit;	
		}else {
			$tengah .= '<div class="error">'.mysql_error().'</div>';	
		}
